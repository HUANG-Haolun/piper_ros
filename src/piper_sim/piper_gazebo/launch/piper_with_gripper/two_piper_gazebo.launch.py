...
def generate_launch_description():
    ...
    urdf_name = "two_piper_description_gazebo.xacro"
    ...
    load_gripper_trajectory_controller_left = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 
             'left_gripper_controller'],
        output='screen'
        )
    
    load_gripper8_trajectory_controller_left = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 
             'left_gripper8_controller'],
        output='screen'
        )
    ...
    load_gripper_trajectory_controller_right = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 
             'right_gripper_controller'],
        output='screen'
        )

    load_gripper8_trajectory_controller_right = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 
             'right_gripper8_controller'],
        output='screen'
        )
    ...
    close_evt2 = RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[load_joint_trajectory_controller_left, 
                         load_gripper_trajectory_controller_left,
                         load_gripper8_trajectory_controller_left,
                         load_joint_trajectory_controller_right,
                         load_gripper_trajectory_controller_right,
                         load_gripper8_trajectory_controller_right,],
            )
    )

    node_gripper_mirror_controller = Node(
        package='piper_gazebo',
        executable='two_joint8_ctrl.py',
        output='screen'
    )
